...
function printAnimalsWithButtons($result) { //prints results from a select statement
	echo "<table cellspacing='0'>";
	echo "<thead><tr><th>Pen #</th><th>Animal Name</th><th>Type</th><th>Hydration</th><th>Fullness</th><th>Hygiene</th><th>Happiness</th><th>Body Size</th><th>Pen Quality</th><th>Max Pen Capacity</th><th>Tend To</th><th>Slaughter!</th></tr></thead><tbody>";

	while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
		echo "<tr><td>" . $row["PEN_ID"] . "</td><td>" . $row["NAME"] . "</td><td>" . $row["TYPE"] . "</td><td>" . $row["HYDRATION"] . "</td><td>" . $row["FULLNESS"] . "</td><td>" . $row["HYGIENE"] . "</td><td>" . $row["HAPPINESS"] . "</td><td>" . $row["BODYSIZE"] . "</td><td>" . $row["QUALITY"] . "</td><td>" . $row["MAXPOPULATION"] . "</td><td>" . "<button id='" . $row['NAME'] . "' class='tendTo' type='button'>Tend To</button>" . "</td><td><form name='" . $row["NAME"] . "' action='zoo.php' method='post'><input type='hidden' name='delanimalname' value='" . $row["NAME"] . "'><input type='submit' name='delButton' value='Slaughter'></form></td></tr>"; //or just use "echo $row[0]"
	}
	echo "</tbody></table>";
}
...
function printAllZoos($result) { //prints results from a select statement
	...
	while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
		echo "<tr><td> <button type='button' class='logon' id='" . $row["NAME"] . "'>Log On</button>" . "</td><td>" . $row["NAME"] . "</td><td>" . $row["OWNERNAME"] . "</td><td>" . $row["CASH"] . "</td><td><form name='" . $row["NAME"] . "' action='zoo.php' method='post'><input type='hidden' name='delzooname' value='" . $row["NAME"] . "'><input type='submit' name='delButton' value='Slaughter'></form>" . "</td></tr>";
	}
    echo "</tbody></table>";
}
?>